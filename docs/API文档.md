...
> 说明：仅支持手机号注册。密码需至少 8 位，含大写字母+小写字母+数字+特殊字符。

**校验规则**：
- phone: 必填，11 位中国大陆手机号（正则 `/^1\d{10}$/`）
- password: 必填，至少 8 位，须含大写字母、小写字母、数字、特殊字符
...
|------|------|------|------|
| content | string | ✅ | 用户问题 |
| kb_id | int | ❌ | 指定知识库 ID，为 null 时使用全库检索 |
...
```
event: followup
data: {"suggestions": ["如何申请退货？", "运费谁承担？", "退款多久到账？"]}
```

> 在 sources 之后、done 之前推送（可选事件）。AI 回答后生成 2-3 个追问建议，引导用户继续对话。
...
```json
{
  "requirement": "用户下单后未收到确认短信，怀疑系统未发送通知"
}
```
...
```json
{
  "message": "知识库已删除，关联文档的 kb_id 已置空"
}
```

> 删除知识库时，关联文档的 `kb_id` 设为 NULL（ON DELETE SET NULL），文档及向量数据保留。
...
> 管理员统计数据概览，包含用户总数、会话数、消息数、文档数、好评率、平均响应时间。

### 7.3 每日提问趋势

```
GET /api/stats/daily_trend?days=7
Authorization: Bearer <token>
```
...
{
  "trend": [
    {"date": "2026-06-17", "count": 45},
    {"date": "2026-06-18", "count": 52},
    {"date": "2026-06-19", "count": 38},
    {"date": "2026-06-20", "count": 61},
    {"date": "2026-06-21", "count": 48},
    {"date": "2026-06-22", "count": 55},
    {"date": "2026-06-23", "count": 67}
  ]
}
```
...
| PATCH | `/api/sessions/{id}/pin` | ✅ | ❌ | 置顶/取消置顶会话 |
| DELETE | `/api/sessions/{id}` | ✅ | ❌ | 删除会话 |
| **POST** | **`/api/chat/{session_id}`** | ✅ | ✅ **SSE** | **发送消息（支持 kb_id）** |
...
| GET | `/api/stats/overview` | ✅ | ❌ | 管理后台概览 |
| GET | `/api/stats/daily_trend` | ✅ | ❌ | 每日提问趋势 |
| GET | `/api/stats/feedback_sessions` | ✅ | ❌ | 会话评价统计 |
| GET | `/api/health` | ❌ | ❌ | 健康检查 |
