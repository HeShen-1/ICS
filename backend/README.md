...
| LLM | DeepSeek API (deepseek-v4-flash) | 云端大模型 |
...
│   ├── agent/               # Agent 任务拆解
│   │   ├── intent.py        #   意图识别分类器
│   │   ├── decomposer.py    #   需求拆解引擎
│   │   └── prompt.py        #   Agent System Prompt
│   ├── api/                 # 路由层
│   │   ├── agent.py         #   Agent 需求拆解接口
│   │   ├── auth.py          #   注册 / 登录 / Token 刷新
│   │   ├── chat.py          #   SSE 流式聊天
│   │   ├── sessions.py      #   会话 CRUD
│   │   ├── knowledge.py     #   文档上传 / 管理 / 知识库 CRUD
│   │   ├── feedback.py      #   用户反馈
│   │   └── stats.py         #   统计数据（概览 / 趋势 / 日报）
│   ├── services/            # 服务层（业务编排）
│   │   ├── auth_service.py
│   │   ├── chat_service.py
│   │   ├── session_service.py
...
| `EMBEDDING_MODEL` | ❌ | 默认 BAAI/bge-m3（也可设为本地路径如 `./models/bge-m3`） |
...
| POST | `/api/agent/decompose` | Agent 需求拆解 |
...
| GET | `/api/stats/daily_trend` | 提问趋势 |
...
# 仅测试 RAG 模块并查看覆盖率
pytest --cov=app.rag tests/test_rag/ -v
```
