...
class TestClassifyIntentWithLLM:
    @patch("app.rag.intent.OpenAI")
    def test_llm_classify_product(self, mock_openai):
        ...
        with patch("app.rag.intent.get_settings") as mock_settings:
            ...
            result = classify_intent("这个产品有什么功能")
            assert result == "产品咨询"

    @patch("app.rag.intent.OpenAI")
    def test_llm_fallback_on_error(self, mock_openai):
        mock_client = MagicMock()
        ...
        with patch("app.rag.intent.get_settings") as mock_settings:
            ...
            result = classify_intent("我想退货")
            assert result == "售后问题"  # keyword fallback

    @patch("app.rag.intent.get_settings")
    def test_falls_back_to_keyword(self, mock_settings):
        mock_settings.return_value = MagicMock(
